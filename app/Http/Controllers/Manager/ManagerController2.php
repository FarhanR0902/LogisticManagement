...
//     }
    ...
    public function gudangOntime(Request $request)
    {
        $query = DB::table('logistik_pengiriman')
            ->whereIn('status', ['On Time', 'ONTIME', 'OnTime', 'on time']);

        if ($request->filled('month')) {
            $query->whereMonth('tanggal_tiba_gudang', $request->bulan);
        }

        if ($request->filled('tahun')) {
            $query->whereYear('tanggal_tiba_gudang', $request->year);
        }
        ...
        $logistik = $query->orderByDesc('tanggal_tiba_gudang')->get();
    ...
    {
        $query = DB::table('logistik_pengiriman')
            ->whereIn('status', ['Delay', 'Critical Delay', 'DELAY', 'delay']);

...
public function dashboard()
{
    ...
    $armada = $shipments->filter(function ($row) {
        ...
        return !empty($row->tanggal_dpt_unit) && !empty($tiba);
    })->count();
    ...
    // =====================================================
    // SLA ONTIME (UNIT vs TIBA GUDANG)
    // =====================================================
    $ontime = $shipments->filter(function ($row) {

        if (empty($row->tanggal_dpt_unit)) return false;
...
// SLA LOGIC (pakai timestamp biar akurat)
if ($start && $end) {
...
public function slaOntime(Request $request)
{
    ...
    return $row;
        ...
        ->filter(function ($row) {
            ...
            $status = strtoupper(trim($row->sla_dapat_mobil));
...
public function slaDelay(Request $request)
{
    ...
    $list = $query->orderBy('tanggal_dpt_unit', 'DESC')
        ->get()
        ->map(function ($row) {

            $dpt = $row->tanggal_dpt_unit
                ? strtotime($row->tanggal_dpt_unit)
                : null;
            ...
            if ($row->tanggal_tiba_gudang) {
                $gudang[1] = strtotime($row->tanggal_tiba_gudang);
            }

            if ($row->tanggal_tiba_gudang_2) {
                $gudang[2] = strtotime($row->tanggal_tiba_gudang_2);
            }

            if ($row->tanggal_tiba_gudang_3) {
                $gudang[3] = strtotime($row->tanggal_tiba_gudang_3);
            }
            ...
            // GUDANG TERCEPAT (dipakai SLA)
            // =========================
            $row->gudang_sla = array_search(min($gudang), $gudang);

            // =========================
            // GUDANG TERLAMBAT (penyebab delay)
            // =========================
            $row->gudang_delay = array_search(max($gudang), $gudang);

           // =========================
// HITUNG DELAY (pakai gudang tercepat)
// =========================
$tiba_tercepat = min($gudang);

$tglDpt  = date('Y-m-d', $dpt);
$tglTiba = date('Y-m-d', $tiba_tercepat);

// ONTIME jika masih hari yang sama
if ($tglTiba <= $tglDpt) {
    return null;
}

$selisihJam = ($tiba_tercepat - $dpt) / 3600;

$row->sla_status = 'DELAY';
$row->selisih_jam = round($selisihJam, 2);

...
class PasuruanImport implements ToModel, WithHeadingRow, WithEvents
{
    ...
    // =====================================================
    // $tarifByRoute: dikelompokkan per ROUTE (sama persis dengan
    // pendekatan di LogistikImport), supaya bisa matching Route +
    // Ekpedisi (exact) + Mobil (prefix match, karena kolom Mobil di
    // Excel sering kepotong dibanding yang lengkap di master_harga).
    // Structure: [ normalized_route => Collection of {ekpedisi, mobil, biaya_kirim} ]
    // =====================================================
    ...
    self::$tarifByRoute = DB::table(self::TARIF_TABLE)
        ...
        //
        // SAMA PERSIS seperti di LogistikImport: matching dilakukan per
        // Route (grouped), lalu di dalam grup itu dicocokkan Ekpedisi
        // (exact match kalau Excel terisi) dan Mobil (PREFIX MATCH, karena
        // kolom Mobil di file Excel sering kepotong dibanding yang lengkap
        // di master_harga, mis. "Contnr 40 Ft Re" vs "Contnr 40 Ft Reefer").
        // =====================================================
        if (self::$tarifByRoute === null) {
            self::$tarifByRoute = DB::table(self::TARIF_TABLE)
                ->select('ekpedisi', 'route', 'mobil', 'biaya_kirim')
...
$biayaKirim = $tarifRow
    ...
    : $this->cleanNumber($row['biaya_kirim_pasuruan'] ?? null);
        ...
        // "Biaya Kirim" dari Excel kalau tidak ada yang cocok.
        // SAMA PERSIS seperti logic biaya_kirim di LogistikImport.
        // =====================================================
        $tarifRow   = $this->findTarif($route, $ekspedisi, $mobil);
        $biayaKirim = $tarifRow
            ? $this->cleanNumberTarif($tarifRow->biaya_kirim)
     ...
     * SAMA PERSIS dengan LogistikImport::normalize().
     */
     ...
     * SAMA PERSIS dengan LogistikImport::normalizeMobil().
     */
     ...
     * + Mobil dari Excel. SAMA PERSIS dengan LogistikImport::findTarif().
     *
     * Strategi:
    ...
    private function findTarif(?string $route, ?string $ekpedisi, ?string $mobil)
    {
        ...
        // Tahap 1: kalau ekpedisi di Excel terisi, coba cocokkan ketat
        // (Route + Ekpedisi exact + Mobil prefix match).
        if ($ekpedisiKey !== '') {
        ...
        // Tahap 2 (fallback): Route + Mobil prefix match saja, abaikan
        // Ekpedisi (kalau Excel kosong, atau nama ekpedisinya beda
        // penulisan dengan yang di master_harga).
     ...
     * SAMA PERSIS dengan LogistikImport::cleanNumberTarif().
     */

...
class LogistikImport implements ToModel, WithHeadingRow
{
    ...
    public function model(array $row)
    {
        ...
        if (
            !empty($row['tanggal_naik_logistik']) &&
            !empty($row['tanggal_dpt_unit'])
        ) {

            $start = strtotime($row['tanggal_naik_logistik']);
            $end   = strtotime($row['tanggal_dpt_unit']);

            $jam = ($end - $start) / 3600;

            $lama_waktu_pencarian = round($jam, 1) . ' Jam';
        ...
        if (
            ...
            !empty($row['tanggal_keluar_gudang'])
        ) {

            $in  = strtotime($row['tanggal_tiba_gudang']);
            $out = strtotime($row['tanggal_keluar_gudang']);

            $jam = ($out - $in) / 3600;

            $lama_digudang = round($jam, 1) . ' Jam';
        ...
        $harga = DB::table('databasehargacsv')

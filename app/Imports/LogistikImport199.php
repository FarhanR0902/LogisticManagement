...
class LogistikImport implements ToModel, WithHeadingRow, WithEvents, WithCalculatedFormulas
{
    ...
    private static $existingKeys = null;

// Duplikat DI DALAM file yang sedang diimport saat ini (jaga-jaga
// kalau baris yang sama kebetulan muncul dua kali di satu file Excel)
private $seenKeysThisImport = [];
...
public function getSkippedCount(): int { return $this->skipped; }
 ...
 if (self::$existingKeys === null) {
            self::$existingKeys = DB::table('logistik_pengiriman')
                ->select('no_shipment', 'tujuan')
                ->get()
                ->mapWithKeys(function ($row) {
                    return [$this->makeDupKey($row->no_shipment, $row->tujuan) => true];
                })
                ->toArray();
        }
    }
...
if ($hasilKubik !== null || $hasilTonase !== null) {
    $pengirimanOptimal = (($hasilKubik >= 85) || ($hasilTonase >= 85))
        ...
        $tujuanKey = preg_replace('/\s+/', ' ', trim(strtolower($tujuan)));
          ...
          if ($noShipment && $tujuan) {
            $dupKey = $this->makeDupKey($noShipment, $tujuan);

            if (isset(self::$existingKeys[$dupKey]) || isset($this->seenKeysThisImport[$dupKey])) {
                $this->skipped++;
                return null;
            }

            $this->seenKeysThisImport[$dupKey] = true;
        }
 ...
 'total_kubik'        => $totalKubik,
    ...
    private function cleanText($value)
    {
      ...
      private function makeDupKey($noShipment, $tujuan): string
    {
        $ns = strtolower(trim((string) $noShipment));
        $tj = preg_replace('/\s+/', ' ', strtolower(trim((string) $tujuan)));

...
<meta charset="utf-8">
<title>SPV Monitoring Dashboard</title>
...
<select name="dist_channel">

    <option value="">-- Dist Channel --</option>

    @foreach($list_dist_channel as $d)

        <option
            value="{{ $d->dist_channel }}"
            {{ request('dist_channel') == $d->dist_channel ? 'selected' : '' }}
        >
            {{ $d->dist_channel }}
        </option>
...
<!-- <a href="{{ url('/logistik') }}?{{ http_build_query(request()->query()) }}" class="card blue">
    <a href="{{ url('/datalogistik') }}?{{ http_build_query(request()->query()) }}" class="card blue"> -->

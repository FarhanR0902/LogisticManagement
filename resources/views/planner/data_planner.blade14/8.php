    ...
    //     let transportLeadTime = row.find('[name="transport_lead_time"]').val();

    //     if (tglNaikInput.val() && transportLeadTime && parseInt(transportLeadTime) > 0) {
    //         rencanaInput.val(
    //             hitungRencanaKirimJS(tglNaikInput.val(), transportLeadTime)
    //         );
    ...
    // }
                        ...
                        // Jalankan sekali di awal untuk semua row yang sudah ada di tabel
                        // $('#tablePlanner tbody tr').each(function() {
                        //     autoRencanaKirim($(this));
                        // });

                        // Hitung ulang otomatis saat via_kirim, tanggal_naik_logistik, ATAU lead time diubah
                    // Hitung ulang otomatis saat tanggal_naik_logistik ATAU lead time diubah
    // $(document).on(
    //     'input change',
    //     '#tablePlanner [name="tanggal_naik_logistik"], #tablePlanner [name="transport_lead_time"]',
    //     function() {
    //         let row = $(this).closest('tr');
    //         autoRencanaKirim(row);
    //         row.find('[name="rencana_kirim"]').trigger('change'); // biar autosave & warna input ikut update
    //     }
    // );
                        // ========================================================
                        // ========================================================
    ...
    initComplete: function() {
        ...
        setTimeout(function() {
            ...
            initSelect2Row();   // <-- INI YANG KURANG
            ...
            if (!$(this).hasClass('select2-hidden-accessible')) {
                ...
                if ($(this).is('.row-route, .row-mobil, .row-ekpedisi')) {
                    $(this).off('select2:select.autotarif').on('select2:select.autotarif', function() {
                        ...
                        // Supaya DataTables membaca value semua input
                    ...
                    let saveTimer = {};   // ganti jadi object, supaya per-row
    ...
    $(document).on('change input',
        ...
        function() {
            ...
            clearTimeout(saveTimer[id]);   // clear timer row INI saja
            ...
            saveTimer[id] = setTimeout(function() {
                saveRow(id);
                        ...
                        function hitungSemuaCostRatioTabel() {
                            ...
                            // -----------------------------------------------------
                            // PASS 1: Kumpulkan total nilai muatan (SUM) & biaya kirim (MAX) per shipment
                            // -----------------------------------------------------
                            dt.rows({
                                search: 'applied'
                            }).every(function() {
                                ...
                                // biaya kirim diambil MAX, bukan SUM, karena nilainya sama untuk
                                // semua baris dalam satu no_shipment yang sama
                                shipmentGroups[noShipment].totalBiaya = Math.max(
                                    shipmentGroups[noShipment].totalBiaya,
                                    biaya
                                );
                            ...
                            dt.rows({
                                ...
                                if (noShipment && shipmentGroups[noShipment]) {
                                    ...
                                    if (totalMuatan > 0 && nilaiMuatanBaris > 0) {
                                        ...
                                        // kontribusi muatan baris ini terhadap total muatan shipment
                                        var kontribusi = nilaiMuatanBaris / totalMuatan;

                                        // CR final untuk baris ini
                                        costRatio = kontribusi * totalCR;
                                    }
                                    ...
                                    // tidak ada no_shipment -> hitung sendiri per baris
                                    var nilaiMuatanMurni = ambilAngkaMurni(row.find('.row-nilai-muatan').val());
                                    var biayaMurni = ambilAngkaMurni(row.find('.row-biaya-kirim').val());

...
<!-- PETA SEBARAN -->
<div class="table-box">
    <h3>🗺️ Peta sebaran nilai muatan per area &mdash; {{ strtoupper(session('dist_channel')) }}</h3>
    ...
    <p class="map-note">
        ...
        Koordinat diambil per area di variabel <code>koordinatAreaSales</code>.
    </p>
...
// ================= PETA SEBARAN NILAI MUATAN PER AREA =================
// Sumber data: $summary_area (kolom: area, total_shipment, total_biaya, total_muatan)
const summaryAreaMapSales = @json($summary_area ?? []);

// Koordinat representatif per area (ibu kota provinsi/wilayah).
// Key HARUS sama persis (huruf besar, underscore) dengan nilai kolom area.
...
function normalisasiNamaSales(str){
...
(function isiJumlahAreaPulau(){
    ...
    summaryAreaMapSales.forEach(a => {
        const key = normalisasiNamaSales(a.area);
    ...
    document.querySelectorAll('[data-count-for]').forEach(el => {
        ...
        if (p === '') {
            el.textContent = summaryAreaMapSales.length + ' area';
        } else {
...
// dan session dist_channel yang sedang aktif).
...
function formatRupiahRingkasSales(value){
...
if (document.getElementById('shipmentMap')) {
    ...
    summaryAreaMapSales.forEach(a => {
        ...
        const key = normalisasiNamaSales(a.area);
        const koordinat = koordinatAreaSales[key];

        if (!koordinat) {
            unmatched.push(a.area + ' (Rp ' + Number(a.total_muatan).toLocaleString('id-ID') + ')');
        ...
        const icon = L.divIcon({
            ...
            html: `
                <div style="
                    ...
                    white-space:nowrap;
                ">${formatRupiahRingkasSales(nilaiMuatan)}</div>
            `,
    ...
    if (debugBox) {
        ...
        if (summaryAreaMapSales.length === 0) {
            ...
            html += `<div style="background:#fdeef2;border:1px solid #e11d48;color:#9f1239;
                        ...
                        Cek filter tanggal/area yang aktif, atau kolom <code>area</code>
            ...
            html += `<div style="background:#eef2ff;border:1px solid #4f46e5;color:#3730a3;
                        ...
                        Total area dari controller: <b>${summaryAreaMapSales.length}</b> baris.

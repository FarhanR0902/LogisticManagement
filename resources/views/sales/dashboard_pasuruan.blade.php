...
<title>
    {{ strtoupper(session('dist_channel')) }} Dashboard
</title>
...
/* ================= KARTU "KLAIM" - Nilai Muatan & CR per Pulau =================
   Dipertahankan persis dari versi lama (bukan ikut skema .card di atas)
   supaya section ini tetap kelihatan beda gaya seperti referensi
   dashboard klaim asuransi kamu. */
...
<div class="page-header">
    <div class="page-title">
        <div class="eyebrow">Sales &middot; {{ strtoupper(session('dist_channel')) }}</div>
        <h1>📊 {{ strtoupper(session('dist_channel')) }} Dashboard</h1>
        <p>Ringkasan performa gudang, armada, dan ketepatan waktu pengiriman untuk channel {{ strtoupper(session('dist_channel')) }}.</p>
...
<div class="filter-box">
    ...
    <div class="pulau-section">
        ...
        <div class="pulau-grid" id="pulauGrid">
            ...
            <div class="pulau-box {{ request('pulau') == 'MALUKU' ? 'active' : '' }}" data-pulau="MALUKU">
                <div class="pulau-icon">🌊</div>
                <div class="pulau-name">Maluku</div>
                <div class="pulau-count" data-count-for="MALUKU">-</div>
            </div>
...
<!-- ================= NILAI MUATAN & CR PER PULAU ================= -->
<div class="table-box">

    <h3>🏝️ Nilai muatan &amp; CR per pulau</h3>

    {{-- 3 kartu ringkasan gaya "klaim asuransi": angka besar berwarna,
         dipertahankan sesuai versi sebelumnya supaya section ini tetap
         punya identitas visual yang beda dari card KPI di atas. --}}
    ...
    {{-- Chart kombinasi bar + line: bar biru = Nilai Muatan, line oranye = CR%,
         sumbu kiri Rp / sumbu kanan %. --}}
    ...
    <table>
        ...
        <tr>
            <td>{{ $key+1 }}</td>
            <td>{{ $p->pulau_pasuruan }}</td>
            <td class="num">{{ number_format($p->total_shipment) }}</td>
            <td class="num">Rp {{ number_format($p->total_muatan,0,',','.') }}</td>
            <td class="num">Rp {{ number_format($p->total_biaya,0,',','.') }}</td>
            <td class="num">{{ number_format($p->cr,2) }}%</td>
        </tr>
        @empty
        <tr><td colspan="6" class="empty-row">Belum ada data pulau</td></tr>
        @endforelse
        </tbody>
...
// Data dikirim dari controller manager -> summary_pulau
...
new Chart(document.getElementById('chartPulau'), {
    // Mixed chart (bar + line dalam 1 canvas) tetap butuh "type" di root
...
// Sumber data: $summary_area dari controller manager
...
// yang sedang tampil (hasil filter tanggal/bulan/tahun/area saat ini).

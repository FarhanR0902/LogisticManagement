...
<body>
    ...
    {{ str_contains($text,'H+') ? 'bg-danger text-white' : 'bg-success text-white' }}">
                                        {{ $text }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    @php
                                    $lamaDigudang = $r->lama_digudang;
                                    if (is_numeric($lamaDigudang) && $lamaDigudang != '') {
                                    $hari = floor($lamaDigudang);
                                    $sisaJam = ($lamaDigudang - $hari) * 24;
                                    $jam = floor($sisaJam);
                                    $menit = floor(($sisaJam - $jam) * 60);

                                    if ($hari > 0 && $jam > 0) {
                                    $lamaDigudangText = "{$hari} Hari {$jam} Jam";
                                    } elseif ($hari > 0) {
                                    $lamaDigudangText = "{$hari} Hari";
                                    } elseif ($jam > 0) {
                                    $lamaDigudangText = "{$jam} Jam";
                                    } else {
                                    $lamaDigudangText = "{$menit} Menit";
                                    }
                                    } else {
                                    $lamaDigudangText = $lamaDigudang ?: '-';
                                    }
                                    @endphp
                                    {{ $lamaDigudangText }}
                                </td>
                                ...
                                <td class="text-center">
                                    @php
                                    $statusGudang = trim($r->status_gudang ?? '');
                                    $statusGudang = strtoupper($statusGudang);
                                    @endphp
                                ...
                                <td class="text-center">
                                    @php
                                    $slaLoading = $r->sla_loading;
                                    // Jika format H+2.125, ambil hanya H+2
                                    if (preg_match('/^H\+(\d+)\./', $slaLoading, $matches)) {
                                    $slaLoading = 'H+' . $matches[1];
                                    }
                                    @endphp
                                    @if($slaLoading == 'Sesuai SLA')
                                    <span class="badge bg-success">{{ $slaLoading }}</span>
                                    @elseif(str_contains($slaLoading, 'H+'))
                                    <span class="badge bg-warning text-dark">{{ $slaLoading }}</span>
                                    @else
                                    <span class="badge bg-secondary">{{ $slaLoading ?? '-' }}</span>
                                    @endif
                                </td>

                                <td class="text-center">
                                    @php
                                    $lamaDigudang2 = $r->lama_digudang_2;
                                    if (is_numeric($lamaDigudang2) && $lamaDigudang2 != '') {
                                    $hari = floor($lamaDigudang2);
                                    $sisaJam = ($lamaDigudang2 - $hari) * 24;
                                    $jam = floor($sisaJam);

                                    if ($hari > 0 && $jam > 0) {
                                    $lamaDigudang2Text = "{$hari} Hari {$jam} Jam";
                                    } elseif ($hari > 0) {
                                    $lamaDigudang2Text = "{$hari} Hari";
                                    } elseif ($jam > 0) {
                                    $lamaDigudang2Text = "{$jam} Jam";
                                    } else {
                                    $lamaDigudang2Text = "-";
                                    }
                                    } else {
                                    $lamaDigudang2Text = $lamaDigudang2 ?: '-';
                                    }
                                    @endphp
                                    {{ $lamaDigudang2Text }}
                                </td>
                                ...
                                <td class="text-center">
                                    @php
                                    $slaLoading2 = $r->sla_loading_2;
                                    if (preg_match('/^H\+(\d+)\./', $slaLoading2, $matches)) {
                                    $slaLoading2 = 'H+' . $matches[1];
                                    }
                                    @endphp
                                    @if($slaLoading2 == 'Sesuai SLA')
                                    <span class="badge bg-success">{{ $slaLoading2 }}</span>
                                    @elseif(str_contains($slaLoading2, 'H+'))
                                    <span class="badge bg-warning text-dark">{{ $slaLoading2 }}</span>
                                    @else
                                    <span class="badge bg-secondary">{{ $slaLoading2 ?? '-' }}</span>
                                    @endif
                                </td>
                                ...
                                <td class="text-center">
                                    @php
                                    $lamaDigudang3 = $r->lama_digudang_3;
                                    if (is_numeric($lamaDigudang3) && $lamaDigudang3 != '') {
                                    $hari = floor($lamaDigudang3);
                                    $sisaJam = ($lamaDigudang3 - $hari) * 24;
                                    $jam = floor($sisaJam);

                                    if ($hari > 0 && $jam > 0) {
                                    $lamaDigudang3Text = "{$hari} Hari {$jam} Jam";
                                    } elseif ($hari > 0) {
                                    $lamaDigudang3Text = "{$hari} Hari";
                                    } elseif ($jam > 0) {
                                    $lamaDigudang3Text = "{$jam} Jam";
                                    } else {
                                    $lamaDigudang3Text = "-";
                                    }
                                    } else {
                                    $lamaDigudang3Text = $lamaDigudang3 ?: '-';
                                    }
                                    @endphp
                                    {{ $lamaDigudang3Text }}
                                </td>
                                ...
                                <td class="text-center">
                                    @php
                                    $slaLoading3 = $r->sla_loading_3;
                                    if (preg_match('/^H\+(\d+)\./', $slaLoading3, $matches)) {
                                    $slaLoading3 = 'H+' . $matches[1];
                                    }
                                    @endphp
                                    @if($slaLoading3 == 'Sesuai SLA')
                                    <span class="badge bg-success">{{ $slaLoading3 }}</span>
                                    @elseif(str_contains($slaLoading3, 'H+'))
                                    <span class="badge bg-warning text-dark">{{ $slaLoading3 }}</span>
                                    @else
                                    <span class="badge bg-secondary">{{ $slaLoading3 ?? '-' }}</span>
                                    @endif
                                </td>
